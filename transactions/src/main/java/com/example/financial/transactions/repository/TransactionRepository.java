...
public interface TransactionRepository extends JpaRepository <TransactionCsv, Long> {
    ...
    @Query("""
            ...
            AND t.amount >= 10000
            """)
    List<TransactionCsv> findSuspectTransactionsByYearAndMonth(@Param("year") int year, @Param("month") int month);
    //TODO: separate queries, find suspect accounts and agencies
    // Validate the transactions in the backend, before sending to the frontend
